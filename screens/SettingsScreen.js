...
const isPro = true; // 🔒 Replace later with real Pro check
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.background }}>
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        {/* Active Schedule (based on today's match) */}
        {todaySchedule && (
          <View style={[styles.activeCard, { backgroundColor: theme.colors.card }]}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Today's Active Schedule</Text>
            <Text style={[styles.scheduleText, { color: theme.colors.text, marginTop: 12 }]}>
              {todaySchedule.name}
            </Text>
          </View>
        )}
        ...
        {schedules.length > 0 && (
          <View style={styles.savedSchedulesSection}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Saved Schedules</Text>
            {schedules.map((schedule) => (
              <View key={schedule.id} style={[styles.scheduleCard, { backgroundColor: theme.colors.card }]}>
        ...
        {(isPro || schedules.length === 0) && (
          <AppButton title="Create New Schedule" onPress={handleAddNew} style={{ marginTop: 24 }} />
        )}
