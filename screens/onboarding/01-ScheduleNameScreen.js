...
import OnboardingContainer from "../../components/OnboardingContainer"; // ✅ shared container
...
export default function ScheduleNameScreen({ route }) {
  ...
  const handleNext = () => {
    ...
    const duplicate = schedules.some((s) => {
      const match =
        s.scheduleName?.trim().toLowerCase() === trimmedName.toLowerCase();
      const sameSchedule =
        edit && existingSchedule && s.id === existingSchedule.id;
  ...
  return (
    <OnboardingContainer>
      ...
      <ContinueModal
        ...
        title="Proceed to Period Setup?"
        message="You’ll set exact start and end times for each period next."
        colors={theme.colors}
      />
    </OnboardingContainer>
  );
